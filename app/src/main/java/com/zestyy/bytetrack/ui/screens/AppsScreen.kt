            ...
            items(state.topApps) { app ->
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Column {
                        ...
                        // No hotspot segment here on purpose: an installed app's own usage is
                        // always Wi-Fi or Mobile (see NetworkUsageRepository) - hotspot bytes are
                        // data forwarded to another device, never attributable to an app on this
                        // one, so it doesn't belong on a per-app breakdown.
                        NetworkBreakdownBar(
