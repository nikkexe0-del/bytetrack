...
import com.zestyy.bytetrack.ui.components.LegendDot
...
import com.zestyy.bytetrack.ui.components.UsageTimelineChart
    ...
    LazyColumn(
        ...
        if (isDayView) {
            item {
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Column {
                        Text("Timeline", style = MaterialTheme.typography.titleLarge, color = TextPrimary)
                        Spacer(Modifier.height(4.dp))
                        Row(horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                            LegendDot(WifiColor, "Wi-Fi")
                            LegendDot(MobileColor, "Mobile")
                            LegendDot(HotspotColor, "Hotspot")
                        }
                        Spacer(Modifier.height(12.dp))
                        UsageTimelineChart(buckets = state.hourlyBuckets, modifier = Modifier.fillMaxWidth())
                    }
                }
            }
